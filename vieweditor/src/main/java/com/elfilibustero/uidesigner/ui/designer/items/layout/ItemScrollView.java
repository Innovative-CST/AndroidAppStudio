...
public class ItemScrollView extends ScrollView implements DesignerItem {
	...
	public void measureChild(View child, int parentWidthMeasureSpec, int parentHeightMeasureSpec) {
		...
		parentWidthMeasureSpec = FrameLayout.getChildMeasureSpec(
	...
	public void measureChildWithMargins(
		...
		int childMeasureSpec = FrameLayout.getChildMeasureSpec(
				parentWidthMeasureSpec,
				getPaddingLeft()
						+ getPaddingRight()
				...
				if (action != 1) {
					if (action == MotionEvent.ACTION_MOVE) {
						...
						if (deltaY <= 0) {
							...
							int bottomEdge = firstChild.getBottom()
									- getScrollY()
									- getHeight()
									+ getPaddingRight();
							if (bottomEdge > 0) {
	...
	private int getVerticalScrollRange(Rect rect) {
		...
		if (childCount == 0) {
			...
			if (rect.bottom > bottomEdgeAdjusted && rect.top > topEdge) {
				...
				verticalRange = Math.min(verticalRange, getChildAt(0).getBottom() - bottomEdgeAdjusted);
